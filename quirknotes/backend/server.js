...
app.delete("/deleteAllNotes", express.json(), async (req, res) => {
  try {
    // Find note with given ID
    const collection = db.collection(COLLECTIONS.notes);
    //const count_notes = int(collection.countDocuments());
    var count_notes= await collection.countDocuments();
    const data = await collection.deleteMany();

    if (data.deletedCount === 0) {
      return res
        .status(404)
        .json({ error: "Unable to delete all documents." });
    }
    res.status(200).json({ response: `${count_notes} deleted.` });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
})